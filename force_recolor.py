...
document.addEventListener('DOMContentLoaded', () => {
    const feedbackHTML = `
        <div class="feedback-widget" style="margin-top: 40px; background: rgba(0, 0, 0, 0.02); border: 1px solid var(--border-subtle); border-radius: 12px; padding: 32px 24px; text-align: center; position: relative; overflow: hidden;">
...
for root, dirs, files in os.walk(base_dir):
    for file in files:
        if file == "index.html":
            ...
            # Using str.find and slicing to do an absolute brute force replacement
            # just in case the regex dotall wasn't matching perfectly due to line endings
            start_idx = content.find('<!-- Feedback Widget -->')
            if start_idx != -1:
                end_script_idx = content.find('</script>', start_idx)
                if end_script_idx != -1:
                    # Plus 9 to include the </script> itself
                    content = content[:start_idx] + new_feedback_snippet + content[end_script_idx + 9:]
...
print(f"Brute force replacement applied to {count} calculators.")
